      ...
      <div className="min-h-screen bg-background-secondary dark:bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="text-center mb-16">
            <div className="mx-auto w-20 h-20 bg-primary/20 dark:bg-primary/30 rounded-full flex items-center justify-center mb-6">
              <HelpCircle className="h-10 w-10 text-primary dark:text-primary" />
            </div>
            <h1 className="text-4xl font-bold text-foreground dark:text-foreground mb-4">
              Frequently Asked Questions
            </h1>
            <p className="text-xl text-foreground-muted dark:text-foreground-muted max-w-3xl mx-auto">
            ...
            <div className="max-w-2xl mx-auto mb-8">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 border border-border dark:border-border rounded-lg bg-background dark:bg-background text-foreground dark:text-foreground placeholder-foreground-muted dark:placeholder-foreground-muted focus:ring-2 focus:ring-primary focus:border-transparent"
                />
              </div>
            </div>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                    selectedCategory === category.id
                      ...
                      : 'bg-background dark:bg-background border border-border dark:border-border hover:bg-secondary dark:hover:bg-secondary'
                  }`}
                >
          ...
          <div className="max-w-4xl mx-auto">
            {filteredFAQs.length === 0 ? (
              <div className="text-center py-12">
                <HelpCircle className="mx-auto h-12 w-12 text-foreground-muted mb-4" />
                <h3 className="text-lg font-medium text-foreground dark:text-foreground mb-2">
                  No questions found
                </h3>
                <p className="text-foreground-muted dark:text-foreground-muted">
              ...
              <div className="space-y-4">
                {filteredFAQs.map((faq, index) => (
                  <div
                    key={index}
                    className="bg-background dark:bg-background rounded-lg border border-border dark:border-border overflow-hidden"
                  >
                    <button
                      onClick={() => toggleItem(index)}
                      className="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-secondary dark:hover:bg-secondary transition-colors"
                    >
                      <h3 className="text-lg font-medium text-foreground dark:text-foreground pr-4">
                        {faq.question}
                      </h3>
                      {expandedItems.has(index) ? (
                        <ChevronUp className="h-5 w-5 text-foreground-muted dark:text-foreground-muted flex-shrink-0" />
                      ) : (
                        <ChevronDown className="h-5 w-5 text-foreground-muted dark:text-foreground-muted flex-shrink-0" />
                      )}
                    </button>
                    
                    {expandedItems.has(index) && (
                      <div className="px-6 pb-4">
                        <p className="text-foreground-muted dark:text-foreground-muted leading-relaxed">
                          {faq.answer}
                        </p>
                      </div>
                    )}
          ...
          <div className="mt-16 text-center">
            <div className="bg-primary/10 dark:bg-primary/20 rounded-lg p-8 max-w-2xl mx-auto">
              <h2 className="text-2xl font-bold text-primary dark:text-primary mb-4">
                Still have questions?
              </h2>
              <p className="text-primary dark:text-primary mb-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <a
                  ...
                  className="inline-flex items-center px-6 py-3 border border-primary text-primary dark:text-primary font-medium rounded-lg hover:bg-primary/10 dark:hover:bg-primary/20 transition-colors"
                >
                  Contact Support
                </a>
              </div>
