...
const PromptBuilder: React.FC = () => {
  ...
  const onClickImport = () => {
    if (elements.length > 0) {
      const proceed = window.confirm(
        "Importing will replace the current structure. Continue?"
      );
      if (!proceed) return;
    }
  ...
  useEffect(() => {
    if (selectedElement) {
      const findUpdatedElement = (
        elementsArray: XMLElement[],
        id: string
      ): XMLElement | null => {
      ...
      if (!updatedElement) {
        ...
        JSON.stringify(updatedElement.children) !==
          JSON.stringify(selectedElement.children)
      ) {
  ...
  const deleteElement = (elementId: string) => {
    ...
    const shouldResetSelection = !selectedElement
      ? false
  ...
  const toggleVisibilityElement = (elementId: string) => {
    const toggleVisibilityRecursive = (
      elements: XMLElement[]
    ): XMLElement[] => {
      return elements.map((el) => {
        if (el.id === elementId) {
  ...
  const moveElementUp = (elementId: string) => {
    ...
    const findElementWithParent = (
      elements: XMLElement[],
      parentElements: XMLElement[] | null = null
    ): {
      element: XMLElement;
      parentElements: XMLElement[];
      index: number;
    } | null => {
      for (let i = 0; i < elements.length; i++) {
        ...
        if (elements[i].children.length > 0) {
          const found = findElementWithParent(
            elements[i].children,
            elements[i].children
          );
  ...
  return (
    <div id="prompt-builder" className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card className="p-4 border border-black dark:border-gray-100 bg-secondary dark:bg-gray-800 rounded-sm">
        <h2 className="text-xl font-bold mb-4 flex justify-between items-center border-b-2 border-black dark:border-gray-100 pb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              className="flex items-center gap-1 bg-primary hover:bg-primary-hover text-white font-bold border border-black rounded-none  hover:translate-y-[1px]  transition-all"
            >
        ...
        <div
          className={cn(
            "mb-6 bg-white dark:bg-gray-800 rounded-none border border-black dark:border-gray-100 p-4 h-[40vh] overflow-y-auto font-mono",
        ...
        >
          {elements.length === 0 ? (
            <div className="text-center text-gray-400 text-sm">
              <p>
                No elements yet. Add an element to begin building your prompt.
              </p>
            </div>
          ) : (
            <ElementTree
      ...
      <Card className="p-4 border border-black dark:border-gray-100 bg-secondary dark:bg-gray-800 rounded-sm">
        <h2 className="text-xl font-bold mb-4 flex justify-between items-center border-b-2 border-black dark:border-gray-100 pb-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              size="sm"
              className="flex items-center gap-1 bg-primary hover:bg-primary-hover text-white font-bold border-1 border-black rounded-none  hover:translate-y-[1px]  transition-all"
            >
        ...
        <pre
          ...
          className={cn(
            "relative w-full h-[90vh] overflow-y-auto whitespace-pre-wrap font-mono text-sm bg-white dark:bg-gray-800 border-1 border-black dark:border-gray-100 rounded-none p-4",
        ...
        >
          {elements.length === 0
            ? rawInput || (isDragActive ? "Drop file to import…" : "")
            : outputXML}
        </pre>
      </Card>
    </div>
